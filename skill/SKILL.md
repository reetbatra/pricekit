# pricekit — integrate Dodo credit billing into this app

You are integrating prepaid credit billing (Dodo Payments) into the user's
existing app. pricekit has already provisioned billing objects in TEST MODE;
their IDs are in `.pricekit/blueprint.json`. Your job: wire the app, then prove it.

## Step 0 — Build the app profile (infer before you ask)

Read the repo and fill the `AppProfile` (see `pricekit/src/detect-schema.ts`):
...
Then run: `npx pricekit plan --app <name> --profile '<json>'` and present THE PLAN verbatim.
...
2. Webhook route at `/api/webhook` via the adaptor's `Webhooks` handler with
   ...
   `.pricekit/last-webhook.json` (verify depends on this side-effect).
3. Usage: fire ONE event per AI action with `event_id: crypto.randomUUID()`,
   `event_name` from `.pricekit/blueprint.json`. Never retry with a new id.
...
Run `npx pricekit verify --json`. Fix reds. Re-run.
